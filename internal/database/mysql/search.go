...
func buildWhereClause(args *models.Arguments) (string, []interface{}) {
	...
	if args.Filter != "" {
		conditions = append(conditions, "(LOWER(output) LIKE LOWER(?) OR LOWER(rule) LIKE LOWER(?))")
		params = append(params, "%"+args.Filter+"%", "%"+args.Filter+"%")
	}
	...
	if args.Tags != "" {
		...
		for i, tag := range tags {
			tagConditions[i] = "LOWER(tags) LIKE LOWER(?)"
	...
	if args.Since != "" {
		if t := int64(utils.ConvertToSeconds(args.Since)); t != 0 {
			...
			conditions = append(conditions, "timestamp_utc >= ?")

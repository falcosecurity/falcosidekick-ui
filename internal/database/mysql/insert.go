...
INSERT IGNORE INTO falco_events (
...
// InsertEvent inserts a Falco event into MySQL
func InsertEvent(db *sql.DB, event *models.Event) error {
	...
	// Convert timestamp to microseconds (consistent with Redis)
	timestampNS := event.Time.UnixNano() / 1e3
